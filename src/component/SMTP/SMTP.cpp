...
SMTPClientComponent::SMTPClientComponent(
    ...
    : userver::components::LoggableComponentBase(config, context),
      ...
      _resolver{
          &context
               .FindComponent<userver::clients::dns::Component>("dns-client")
               .GetResolver()
      },
      m_background_tasks{
...
bool SMTPClientComponent::SendEmail(
    ...
    const auto deadline = Deadline::FromDuration(kIoTimeout);
    ...
    try {
        ...
        const auto is_final_response_line = [](std::string_view line) {
            return line.size() >= 4 &&
        ...
        auto expect_response = [&](auto& stream, int expected_code) {
            ...
            const int actual_code =
                std::stoi(response.substr(0, 3));
        ...
        const auto resolved = _resolver->Resolve(
            m_host,
            deadline
        );
        ...
        if (resolved.empty()) {
            throw std::runtime_error(
                "DNS resolution failed for host: " + m_host
            );
        }

        userver::engine::io::Sockaddr address;
        bool found_ipv4 = false;

        for (const auto& addr : resolved) {
            if (
                addr.Domain() ==
                userver::engine::io::AddrDomain::kInet
            ) {
                address = addr;
                found_ipv4 = true;
                break;
        ...
        if (!socket.IsValid()) {
            throw std::runtime_error(
                "Failed to initialize SMTP socket"
            );
        }
...
SMTPClientComponent::GetStaticConfigSchema() {
