...
struct ProjectRowPopulated final {
    ProjectRow project;
    priemman::common::PublicInfo author;
};
...
struct ProjectMediaRow {
    ...
    std::string cloudinary_public_id;
};
...
public:
    ...
    std::optional<ProjectRowPopulated> FindById(const std::string& id) const;
    bool ExistsByOwnerSlug(const std::string& owner_id, const std::string& slug) const;

    ProjectRowPopulated Create(const ProjectRow& row) const;

    std::optional<ProjectRowPopulated> Update(const ProjectRow& row) const;
    ...
    std::vector<ProjectRowPopulated> ListByOwner(const std::string& owner_id,
                                        const std::string& status_filter,
                                        std::int64_t limit, std::int64_t offset) const;
    std::vector<ProjectRowPopulated> ListPublic(std::int64_t limit, std::int64_t offset) const;
