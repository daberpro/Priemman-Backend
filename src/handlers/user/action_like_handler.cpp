#include "action_like_handler.hpp"
...
namespace priemman::handlers::user {
    ...
    std::string ActionLikeHandler::HandleRequestThrow(
        ...
        if (method == HttpMethod::kGet) {
            ...
            const auto rows = _users.ListLikedProjects(*user_id, limit, offset);
        ...
        // Gabungkan POST dan DELETE karena validasi payload dan entitasnya persis sama
        if (method == HttpMethod::kPost || method == HttpMethod::kDelete) {
            ...
            if (method == HttpMethod::kPost) {
                if (_users.ActionLike(user->id, body_project_id)) {
                    result_msg += std::format("Liked project #{}. ", body_project_id);
                    action_executed = true;
                }
            } else {
                if (_users.ActionUnLike(user->id, body_project_id)) {
                    result_msg += std::format("Unliked project #{}. ", body_project_id);
                    action_executed = true;
                }
            }
